...
new_width = 1127;
new_height = 845;
resizedip_image = uint8(imresize(input_image,[new_height,new_width],"bicubic"));
...
%saving image files
imwrite(distorted_image, 'Distorted_image_scenic.jpg')

% Initialize the undistorted image with the same dimensions and type
undistorted_image = uint8(zeros(size(resizedip_image)));

k_1 = 0.2011; % Average value of= 0.2011   manual= 0.172 best PSNR = 0.191 , best SSIM = 0.197
%k1_val = 0.1:0.001:0.25;
%n_val = numel(k1_val);

k_2 = -0.02687; % manual= -0.00985 best SSIM = -0.016
...
% Apply the inverse distortion to correct
    ...
    %undistorted_image = imresize(undistorted_image,[new_height,new_width],"bicubic");    
...
title('Corrected Image of cboard');
...
%ylabel ("SSIM");
%title("SSIM values for differnt pincushon distortion correction coefficient(K\_2)");
%grid on;
...
%Calculating Mean Squareed Error(MSE) for each pixle
    mse = mean((sharpened_image - resizedip_image).^2, "all"); % mse(n) = mean((undistorted_image - resizedip_image).^2, "all");
    
    %Claculating the Peak Sigmal To Nise Ratio (PSNR)
    maxPixelValue = double(max(resizedip_image,[],'all')); 
    psnr = 10*log10(maxPixelValue^2/mse); % psnr(n) = 10*log10(maxPixelValue^2/mse(n));
   
    %Calculating Structural Similarity Index
    ssim_val = ssim(resizedip_image,undistorted_image); %ssim_val(n) = ssim(resizedip_image,undistorted_image);
...
%imwrite(final_image_croped, 'corrected_image_scenic_manual.jpeg')
